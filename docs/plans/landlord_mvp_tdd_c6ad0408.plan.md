...
- [Legal and engineering follow-ups / checklist](docs/discussions/landlord_mvp_tdd_plan_review_followups.md) — expanded themes below are merged from this doc
...
Align with the spec’s [File Structure](docs/specs/landlord_case_organizer_mvp_design_spec.md) and keep **domain + export + persistence testable without the UI**:
...
- **GREEN**: thin module around Tesseract.js per [Technical Stack](docs/specs/landlord_case_organizer_mvp_design_spec.md).
...
Tests mirror [Legal Knowledge Organizer](docs/specs/landlord_case_organizer_mvp_design_spec.md) workflow only as far as MVP requires; link each describe block to a spec heading.

### Phase 7 — Markdown export

- **RED**: fixture case → snapshot or exact string for **Full Case File** and at least one variant (**Timeline Only** or **Evidence Only**) per [Export System](docs/specs/landlord_case_organizer_mvp_design_spec.md).
